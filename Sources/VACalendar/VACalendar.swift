...
protocol VACalendarDelegate: AnyObject {
...
public class VACalendar {
    ...
    public init(
        ...
        _startDate = startDate ?? calendar.date(byAdding: .year, value: -1, to: Date())
        _endDate = endDate ?? calendar.date(byAdding: .year, value: 1, to: Date())
        months = generateMonths(from: _startDate!, endDate: _endDate!)
    }
